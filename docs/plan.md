...
- [ ] S050: Current backend lifecycle contract is still skeleton-oriented: `IRxVideoBackend::start_video(...)`, `IRxAudioBackend::start_audio(...)`, and `IRxBackend::stop()` are still `void`, so backend-visible lifecycle misuse and operational failure propagation are not yet modeled through an explicit common result/state boundary. Task `035` already extended the common `Error` model with backend/runtime operational categories, but task `036` is still required to make backend lifecycle/state transitions explicit instead of relying on silent no-op behavior, ad hoc logging, or implicit contracts.
