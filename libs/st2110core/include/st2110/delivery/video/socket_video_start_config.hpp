...
inline SocketVideoStartConfig project_receive_start_request_to_socket_video_start(const ReceiveStartRequest &request, const Settings settings) {
    ...
    res.video_receive_pipeline_config = make_video_receive_pipeline_config(res.stream.scan_mode, res.stream.media, settings.partial_unit_policy);
    ...
    for (std::size_t i = 0; i < bootstrap.receive_bootstrap.legs.size(); ++i) {
        res.legs.emplace_back(request.local.legs[i].family, request.local.legs[i].local_ip,
                              ...
                              bootstrap.receive_bootstrap.legs[i].max_udp_datagram_bytes);
    }

    const auto& open_cfg = build_socket_rx_open_config(res.legs[0].udp_port, res.legs[0].local_ip, res.legs[0].dest_ip);
    if (!open_cfg) {
        throw std::runtime_error("Can not build socket open config");
    }
    res.open_config = *open_cfg;
