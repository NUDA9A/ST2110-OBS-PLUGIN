    ...
    if (line.starts_with("m=")) {
      ...
      if (line.starts_with("m=video")) {
        ...
        if (contains_payload_type(*payload_types, expected_payload_type)) {
          if (found) {
            duplicate_candidate = RawVideoSdpDuplicateMediaCandidate{.media_line = std::string(line),
      ...
      if (line.starts_with("c=")) {
        ...
        if (ts_refclk.has_value()) {
          if (Error err = set_raw_sdp_scoped_timing_attribute(res.session_ts_refclk, res.ts_refclk, *ts_refclk,
        ...
        if (tp.has_value()) {
          if (Error err =
                  set_raw_sdp_scoped_timing_attribute(res.session_tp, res.tp, *tp, RawSdpAttributeScope::Session);
              err != Error::Ok) {
            return std::unexpected(err);
          }
        ...
        if (cmax.has_value()) {
          if (Error err =
                  set_raw_sdp_scoped_timing_attribute(res.session_cmax, res.cmax, *cmax, RawSdpAttributeScope::Session);
              err != Error::Ok) {
            return std::unexpected(err);
          }
      ...
      if (line.starts_with("c=")) {
        ...
        if (tp.has_value()) {
          if (Error err = set_raw_sdp_scoped_timing_attribute(res.media_tp, res.tp, *tp, RawSdpAttributeScope::Media);
              err != Error::Ok) {
            return std::unexpected(err);
          }
        ...
        if (troff.has_value()) {
          if (Error err =
                  set_raw_sdp_scoped_timing_attribute(res.media_troff, res.troff, *troff, RawSdpAttributeScope::Media);
              err != Error::Ok) {
            return std::unexpected(err);
          }
        ...
        if (cmax.has_value()) {
          if (Error err =
                  set_raw_sdp_scoped_timing_attribute(res.media_cmax, res.cmax, *cmax, RawSdpAttributeScope::Media);
              err != Error::Ok) {
            return std::unexpected(err);
          }
