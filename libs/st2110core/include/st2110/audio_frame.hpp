...
namespace st2110 {
    enum class AudioSampleStorageFormat {
        InterleavedS32
    };
    ...
    public:
        AudioBuffer(
                ...
                : sampling_rate_hz_(sampling_rate_hz),
                  ...
                  samples_(checked_total_sample_count(
                          channel_count,
                          samples_per_channel,
                          storage_format)) {}
        ...
        [[nodiscard]] AudioSampleStorageFormat storage_format() const {
            return format_;
        }

        [[nodiscard]] uint32_t sampling_rate_hz() const {
            return sampling_rate_hz_;
        }

        [[nodiscard]] uint16_t channel_count() const {
            return channel_count_;
        }

        [[nodiscard]] uint32_t samples_per_channel() const {
        ...
        [[nodiscard]] std::size_t total_sample_count() const {
            return samples_.size();
        }

        [[nodiscard]] std::size_t size_bytes() const {
            return total_sample_count() * sizeof(std::int32_t);
        }

        [[nodiscard]] std::int32_t* samples() {
            return samples_.data();
        }

        [[nodiscard]] const std::int32_t* samples() const {
            return samples_.data();
        }
    ...
    private:
        [[nodiscard]] static std::size_t checked_total_sample_count(
                uint16_t channel_count,
                uint32_t samples_per_channel,
            ...
            if (channels != 0 &&
                samples > (std::numeric_limits<std::size_t>::max() / channels)) {
        ...
        [[nodiscard]] std::size_t linear_sample_index(
            ...
            return (static_cast<std::size_t>(sample_index) *
