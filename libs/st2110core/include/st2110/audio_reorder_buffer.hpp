...
namespace st2110 {
    ...
    inline AudioFixedWindowReorderBuffer::AudioFixedWindowReorderBuffer(
    ...
    [[nodiscard]] inline uint16_t AudioFixedWindowReorderBuffer::forward_seq_distance(
            uint16_t from,
            uint16_t to) {
        return static_cast<uint16_t>(to - from);
    }

    [[nodiscard]] inline StoredAudioRtpPacket AudioFixedWindowReorderBuffer::store_packet(
            const AudioRtpPacketView& packet) {
    ...
    [[nodiscard]] inline Error AudioFixedWindowReorderBuffer::push(
            const AudioRtpPacketView& packet) {
        if (!config_is_valid()) {
    ...
    [[nodiscard]] inline std::optional<StoredAudioRtpPacket>
    AudioFixedWindowReorderBuffer::pop_next() {
        if (!expected_seq_) {
            return std::nullopt;
        }
    ...
    [[nodiscard]] inline bool AudioFixedWindowReorderBuffer::has_pending() const {
        return !pending_.empty();
    }

    [[nodiscard]] inline const AudioReorderBufferStats&
    AudioFixedWindowReorderBuffer::stats() const {
        return stats_;
    }
}
