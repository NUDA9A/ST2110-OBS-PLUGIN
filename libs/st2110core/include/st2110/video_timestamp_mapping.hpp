...
namespace st2110 {
    ...
    videoTimestampNanosecondsPerSecond = 1000000000ULL;
    ...
    [[nodiscard]] inline std::expected <TimestampNs, Error> rtp_ticks_to_timestamp_ns(
            uint64_t ticks,
        ...
        auto seconds_ns = checked_video_timestamp_mul_u64(
                seconds,
        ...
        const uint64_t remainder_ns =
                (remainder_ticks * videoTimestampNanosecondsPerSecond) / rtp_clock_rate;
        ...
        auto absolute_ns = checked_video_timestamp_add_u64(
                anchor_timestamp_ns,
                *relative_ns
        );
...
public:
    ...
    [[nodiscard]] std::expected <TimestampNs, Error> map(uint32_t rtp_timestamp) {
        ...
        auto delta = forward_rtp_timestamp_delta(
        ...
        return rtp_ticks_to_timestamp_ns(
                ticks_since_anchor_,
                cfg_.rtp_clock_rate,
                cfg_.anchor_timestamp_ns
        );
    }
...
[[nodiscard]] inline Error validate_synthetic_video_timestamp_mapper_config(
        const SyntheticVideoTimestampMapperConfig &cfg
) {
...
[[nodiscard]] inline std::expected <TimestampNs, Error> synthetic_unit_index_to_timestamp_ns(
        uint64_t unit_index,
        const SyntheticVideoTimestampMapperConfig &cfg
) {
    if (Error err = validate_synthetic_video_timestamp_mapper_config(cfg);
            err != Error::Ok) {
    ...
    const __uint128_t numerator =
                static_cast<__uint128_t>(unit_index) *
                static_cast<__uint128_t>(cfg.fps_den) *
                static_cast<__uint128_t>(videoTimestampNanosecondsPerSecond);

        const __uint128_t relative_ns =
                numerator / static_cast<__uint128_t>(cfg.fps_num);

        const __uint128_t absolute_ns =
                relative_ns + static_cast<__uint128_t>(cfg.anchor_timestamp_ns);
    ...
    auto whole_seconds = checked_video_timestamp_mul_u64(
            whole_groups,
            cfg.fps_den
    );
    ...
    auto whole_ns = checked_video_timestamp_mul_u64(
            *whole_seconds,
            videoTimestampNanosecondsPerSecond
    );
    ...
    auto fractional_num = checked_video_timestamp_mul_u64(
            remainder_units,
            cfg.fps_den
    );
    ...
    fractional_num = checked_video_timestamp_mul_u64(
            *fractional_num,
            videoTimestampNanosecondsPerSecond
    );
...
public:
    explicit SyntheticVideoTimestampMapper(SyntheticVideoTimestampMapperConfig cfg)
            : cfg_(cfg),
              config_error_(validate_synthetic_video_timestamp_mapper_config(cfg_)) {
    }
