...
[[nodiscard]] inline Error validate_audio_rtp_timestamp_mapper_config(
...
forward_audio_rtp_timestamp_delta(uint32_t previous, uint32_t current) {
...
audio_rtp_ticks_to_timestamp_ns(uint64_t ticks, uint32_t rtp_clock_rate) {
  if (rtp_clock_rate == 0) {
    return std::unexpected(Error::InvalidValue);
  }
  ...
  const uint64_t remainder_ns =
      (remainder_ticks * audioTimestampNanosecondsPerSecond) / rtp_clock_rate;
...
public:
  explicit AudioRtpTimestampMapper(AudioRtpTimestampMapperConfig cfg)
      : cfg_(cfg) {}
  ...
  [[nodiscard]] std::expected<TimestampNs, Error> map(uint32_t rtp_timestamp) {
    ...
    if (!has_last_timestamp_) {
      const auto delta = forward_audio_rtp_timestamp_delta(
          cfg_.anchor_rtp_timestamp, rtp_timestamp);
      if (!delta.has_value()) {
      ...
      const auto delta =
          forward_audio_rtp_timestamp_delta(last_rtp_timestamp_, rtp_timestamp);
      if (!delta.has_value()) {
        return std::unexpected(delta.error());
      }

      delta_ticks = *delta;
      if (accumulated_ticks_since_anchor_ >
          std::numeric_limits<uint64_t>::max() - delta_ticks) {
        return std::unexpected(Error::InvalidValue);
      }
    ...
    const auto offset_ns = audio_rtp_ticks_to_timestamp_ns(
        accumulated_ticks_since_anchor_, cfg_.rtp_clock_rate);
    if (!offset_ns.has_value()) {
      return std::unexpected(offset_ns.error());
    }

    if (cfg_.anchor_timestamp_ns >
        std::numeric_limits<TimestampNs>::max() - *offset_ns) {
      return std::unexpected(Error::InvalidValue);
    }
...
[[nodiscard]] inline Error validate_audio_receiver_playout_timing_config(
...
audio_receiver_playout_timing_decision(
  ...
  if (media_timestamp_ns >
      std::numeric_limits<TimestampNs>::max() - cfg.playout_delay_ns) {
    return std::unexpected(Error::InvalidValue);
  }

  return AudioReceiverPlayoutTimingDecision{
...
audio_block_timing(uint32_t rtp_timestamp, TimestampNs media_timestamp_ns,
                   const AudioReceiverPlayoutTimingConfig &cfg) {
  const auto decision =
      audio_receiver_playout_timing_decision(media_timestamp_ns, cfg);
