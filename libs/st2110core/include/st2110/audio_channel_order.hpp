...
namespace st2110 {
    ...
    [[nodiscard]] inline bool is_audio_channel_order_digit(char c) {
        return c >= '0' && c <= '9';
    }
    ...
    parse_audio_channel_order_u_two_digit_count(std::string_view symbol) {
        if (symbol.size() != 3 ||
            symbol[0] != 'U' ||
            !is_audio_channel_order_digit(symbol[1]) ||
            !is_audio_channel_order_digit(symbol[2])) {
            return std::unexpected(Error::InvalidValue);
        }

        const uint16_t value = static_cast<uint16_t>(
                (static_cast<uint16_t>(symbol[1] - '0') * 10U) +
    ...
    parse_smpte2110_audio_channel_order_raw_value(std::string_view raw_value) {
        ...
        while (group_start <= groups_text.size()) {
            ...
            const uint32_t next_declared_count =
                    static_cast<uint32_t>(parsed.declared_channel_count) +
                    static_cast<uint32_t>(group->channel_count);
    ...
    parse_audio_channel_order_signaling(const AudioChannelOrderSignaling& channel_order) {
        if (Error err = validate_audio_channel_order_signaling(channel_order);
                err != Error::Ok) {
        ...
        switch (channel_order.convention) {
            ...
            case AudioChannelOrderConvention::Smpte2110:
                return parse_smpte2110_audio_channel_order_raw_value(
                        channel_order.raw_value);
    ...
    [[nodiscard]] inline Error validate_audio_channel_order_against_channel_count(
    ...
    effective_audio_channel_order_from_audio_stream_signaling(
        ...
        if (Error err = validate_audio_media_description_against_conformance_range(
                    signaling.media,
        ...
        if (Error err = validate_audio_channel_order_against_channel_count(
                    *signaling.channel_order,
                    channel_count);
                err != Error::Ok) {
            return std::unexpected(err);
        }
        ...
        if (parsed->declared_channel_count < channel_count) {
            const uint16_t undefined_remainder =
                    static_cast<uint16_t>(channel_count - parsed->declared_channel_count);

            auto undefined_group =
                    make_audio_channel_order_undefined_group(undefined_remainder);
