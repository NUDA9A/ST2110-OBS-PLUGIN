...
namespace st2110 {
    ...
    [[nodiscard]] inline constexpr Error validate_audio_conformance_range(
    ...
    [[nodiscard]] inline constexpr bool audio_media_description_matches_conformance_range(
        ...
        if (range.min_channels > media.channel_count ||
            range.max_channels < media.channel_count) {
            return false;
        }
    ...
    [[nodiscard]] inline bool audio_signaling_channel_order_is_digit(char c) {
        return c >= '0' && c <= '9';
    }

    [[nodiscard]] inline bool audio_signaling_channel_order_token_contains_ws(
    ...
    audio_signaling_smpte2110_channel_order_symbol_count(std::string_view symbol) {
        ...
        if (symbol.starts_with("U")) {
            if (symbol.size() != 3 ||
                !audio_signaling_channel_order_is_digit(symbol[1]) ||
    ...
    validate_smpte2110_audio_channel_order_raw_value_and_count(
        ...
        while (group_start <= groups_text.size()) {
            ...
            auto group_count =
                    audio_signaling_smpte2110_channel_order_symbol_count(symbol);
    ...
    [[nodiscard]] inline Error validate_audio_stream_signaling(
        ...
        if (signaling.channel_order) {
            if (Error err = validate_audio_channel_order_signaling(*signaling.channel_order);
                    err != Error::Ok) {
                return err;
            }

            if (signaling.channel_order->convention ==
                AudioChannelOrderConvention::Smpte2110) {
                auto parsed =
                        validate_smpte2110_audio_channel_order_raw_value_and_count(
                                signaling.channel_order->raw_value);

                if (parsed.error != Error::Ok) {
                    return parsed.error;
                }

                if (parsed.has_declared_channel_count &&
                    parsed.declared_channel_count > signaling.media.channel_count) {
                    return Error::InvalidValue;
                }
            }
        }
