#ifndef ST2110_OBS_VIDEO_RECEIVER_BOOTSTRAP_HPP
#define ST2110_OBS_VIDEO_RECEIVER_BOOTSTRAP_HPP
...
[[nodiscard]] inline PacketParsePolicy
packet_parse_policy_from_video_stream_signaling(const VideoStreamSignaling &signaling) {
    return PacketParsePolicy{signaling.max_udp_datagram_bytes};
}

[[nodiscard]] inline Error validate_video_stream_signaling_against_rx_video_config(const VideoStreamSignaling &signaling,
...
depacketizer_config_from_video_stream_signaling(const VideoStreamSignaling &signaling, PartialFramePolicy policy) {
    ...
    if (!expected_format.has_value()) {
        return std::unexpected(expected_format.error());
    }
...
[[nodiscard]] inline std::expected<VideoUnitReconstructorConfig, Error>
...
[[nodiscard]] inline std::expected<RxVideoConfig, Error>
...
video_receiver_bootstrap_config_from_video_stream_signaling(const VideoStreamSignaling &signaling, uint16_t udp_port,
    ...
    if (!expected_rx_config.has_value()) {
        return std::unexpected(expected_rx_config.error());
    }
    ...
    if (!expected_receive_pipeline.has_value()) {
        return std::unexpected(expected_receive_pipeline.error());
    }
    ...
    if (!expected_timestamp_mapper_config.has_value()) {
        return std::unexpected(expected_timestamp_mapper_config.error());
    }
...
#endif // ST2110_OBS_VIDEO_RECEIVER_BOOTSTRAP_HPP