...
namespace st2110 {
    ...
    [[nodiscard]] inline bool audio_sdp_ascii_iequals(
            std::string_view lhs,
            std::string_view rhs) {
    ...
    audio_pcm_encoding_from_raw_audio_sdp_rtpmap_encoding_name(
        ...
        if (audio_sdp_ascii_iequals(encoding_name, "L24") ||
            audio_sdp_ascii_iequals(encoding_name, "L16")) {
    ...
    audio_stream_signaling_from_raw_audio_sdp_media_section(
        ...
        if (raw.rtpmap.empty() ||
            raw.parsed_rtpmap.encoding_name.empty() ||
            raw.parsed_rtpmap.sampling_rate_hz == 0) {
            return std::unexpected(Error::InvalidValue);
        }
        ...
        auto encoding = audio_pcm_encoding_from_raw_audio_sdp_rtpmap_encoding_name(
                raw.parsed_rtpmap.encoding_name);
        ...
        if (Error err = validate_audio_stream_signaling(signaling);
                err != Error::Ok) {
