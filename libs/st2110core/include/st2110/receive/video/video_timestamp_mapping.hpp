...
namespace st2110 {
[[nodiscard]] inline std::expected<std::uint64_t, Error> checked_video_timestamp_add_u64(const std::uint64_t a,
...
[[nodiscard]] inline std::expected<std::uint64_t, Error> checked_video_timestamp_mul_u64(const std::uint64_t a,
                                                                                         const std::uint64_t b) {
...
rtp_ticks_to_timestamp_ns(const std::uint64_t ticks, const std::uint32_t rtp_clock_rate,
    ...
    const std::uint64_t remainder_ns = (remainder_ticks * videoTimestampNanosecondsPerSecond) / rtp_clock_rate;
...
class VideoRtpTimestampMapper {
  public:
    explicit VideoRtpTimestampMapper(const VideoRtpTimestampMapperConfig &cfg) : cfg_(cfg) {}
