...
namespace st2110 {
    ...
    [[nodiscard]] inline Error validate_video_receiver_playout_timing_config(
    ...
    video_reconstructed_frame_timing(
            const ReconstructedVideoFrame& frame,
            TimestampNs media_timestamp_ns,
            const VideoReceiverPlayoutTimingConfig& cfg
    ) {
        if (Error err = validate_video_receiver_playout_timing_config(cfg); err != Error::Ok) {
        ...
        return VideoReconstructedFrameTiming{
            frame.rtp_timestamp, timing_decision.media_timestamp_ns, timing_decision.reconstruction_timestamp_ns
        };
    }
}
