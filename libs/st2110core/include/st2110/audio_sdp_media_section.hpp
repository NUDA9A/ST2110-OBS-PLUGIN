...
namespace st2110 {
    ...
    [[nodiscard]] inline bool is_audio_sdp_ascii_ws(char c) {
        return c == ' ' || c == '\t';
    }
    ...
    parse_audio_sdp_uint64(std::string_view text) {
        if (text.empty()) {
    ...
    parse_audio_payload_type(std::string_view text) {
    ...
    [[nodiscard]] inline bool contains_audio_payload_type(
    ...
    parse_audio_attribute_value(std::string_view line, std::string_view prefix) {
        line = strip_audio_sdp_cr(line);
    ...
    [[nodiscard]] inline RawAudioSdpAttribute
    parse_unknown_audio_sdp_attribute(std::string_view line) {
        line = strip_audio_sdp_cr(line);
        ...
        if (line.starts_with("c=")) {
            return RawAudioSdpAttribute{
                    .name = "c",
                    .value = std::string(line.substr(2))
            };
        }
        ...
        return RawAudioSdpAttribute{
                .name = std::string(line.substr(0, eq_pos)),
                .value = std::string(line.substr(eq_pos + 1))
        };
    }

    [[nodiscard]] inline std::expected<RawAudioSdpRtpMap, Error>
    parse_audio_sdp_rtpmap_payload(std::string_view payload) {
        ...
        if (!sampling_rate.has_value() ||
            *sampling_rate == 0 ||
            *sampling_rate > std::numeric_limits<uint32_t>::max()) {
            return std::unexpected(Error::InvalidValue);
        }
    ...
    parse_audio_sdp_ptime_us(std::string_view value) {
        ...
        if (dot_pos != std::string_view::npos &&
            value.find('.', dot_pos + 1) != std::string_view::npos) {
            return std::unexpected(Error::InvalidValue);
        }

        const std::string_view integer_ms_text =
                dot_pos == std::string_view::npos
                ? value
                : value.substr(0, dot_pos);
        ...
        if (packet_time_us == 0 ||
            packet_time_us > std::numeric_limits<uint32_t>::max()) {
            return std::unexpected(Error::InvalidValue);
        }
    ...
    parse_audio_sdp_fmtp_parameter_token(std::string_view parameter) {
        ...
        if (eq_pos == std::string_view::npos) {
            ...
            return RawAudioSdpFmtpParameterToken{
                    .name = parameter,
                    .value = {},
                    .has_value = false
            };
        ...
        if (audio_sdp_fmtp_token_contains_ws(name) ||
            trim_audio_sdp_ascii_ws(value) != value) {
            return std::unexpected(Error::InvalidValue);
        }
    ...
    select_raw_audio_sdp_media_section(
        ...
        while (line_start <= sdp.size()) {
            ...
            if (line.starts_with("m=")) {
                ...
                if (line.starts_with("a=")) {
                    res.unknown_session_attributes.push_back(
                            parse_unknown_audio_sdp_attribute(line));
                }
            } else if (inside_selected_media_section) {
                bool handled_attribute = false;

                if (line.starts_with("c=")) {
                    res.unknown_attributes.push_back(
                            parse_unknown_audio_sdp_attribute(line));
                    handled_attribute = true;
                }

                auto rtpmap = parse_audio_payload_bound_attribute_value(
                        line,
                        "a=rtpmap:",
                ...
                auto fmtp = parse_audio_payload_bound_attribute_value(
                        line,
                        "a=fmtp:",
                        expected_payload_type);
