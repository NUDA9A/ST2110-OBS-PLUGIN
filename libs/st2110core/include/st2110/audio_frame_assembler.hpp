...
namespace st2110 {
    ...
    [[nodiscard]] inline Error validate_audio_frame_assembler_config(
            const AudioFrameAssemblerConfig& cfg) {
    ...
    [[nodiscard]] inline std::expected<int32_t, Error>
    decode_audio_pcm_wire_sample_to_s32(
        ...
        switch (wire_format) {
            case AudioPcmWireFormat::L16: {
                ...
                const uint32_t raw =
                        (static_cast<uint32_t>(sample_bytes[0]) << 8U) |
                        static_cast<uint32_t>(sample_bytes[1]);
            ...
            case AudioPcmWireFormat::L24: {
                ...
                const uint32_t raw =
                        (static_cast<uint32_t>(sample_bytes[0]) << 16U) |
                        (static_cast<uint32_t>(sample_bytes[1]) << 8U) |
                        static_cast<uint32_t>(sample_bytes[2]);
    ...
    public:
        explicit AudioFrameAssembler(AudioFrameAssemblerConfig cfg = {})
                : cfg_(cfg) {
        }

        [[nodiscard]] std::expected<AssembledAudioBlock, Error>
        push(const AudioRtpPacketView& packet) {
            const auto reject = [this](Error error)
                    -> std::expected<AssembledAudioBlock, Error> {
            ...
            if (packet.sampling_rate_hz == 0 ||
                packet.channel_count == 0 ||
                packet.samples_per_channel == 0) {
                return reject(Error::InvalidValue);
            }

            const auto wire_sample_bytes =
                    audio_pcm_wire_sample_bytes(packet.wire_format);
            ...
            const auto expected_payload_size =
                    checked_audio_assembler_payload_size_bytes(
                            ...
                            static_cast<std::size_t>(*wire_sample_bytes));
            ...
            for (uint32_t sample_index = 0;
                 sample_index < packet.samples_per_channel;
                 ++sample_index) {
                for (uint16_t channel = 0;
                     channel < packet.channel_count;
                     ++channel) {
                    const std::size_t sample_ordinal =
                            static_cast<std::size_t>(sample_index) *
                            static_cast<std::size_t>(packet.channel_count) +
        ...
        [[nodiscard]] const AudioFrameAssemblerStats& stats() const {
            return stats_;
        }
