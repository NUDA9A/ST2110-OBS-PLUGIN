...
public:
  ...
  [[nodiscard]] std::expected<AssembledAudioBlock, Error> push(const AudioRtpPacketView &packet) {
    ...
    AudioBuffer buffer{packet.sampling_rate_hz, packet.channel_count, packet.samples_per_channel, cfg_.storage_format};
    ...
    return AssembledAudioBlock{std::move(buffer), packet.rtp.timestamp, packet.rtp.seq_number, packet.rtp.marker, true};
  }
