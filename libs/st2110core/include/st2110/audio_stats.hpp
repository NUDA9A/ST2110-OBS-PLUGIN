...
namespace st2110 {
    ...
    enum class AudioBlockCompletionStatus {
        Complete,
        Partial,
        Dropped
    };

    [[nodiscard]] inline Error validate_audio_block_completion_status(
            AudioBlockCompletionStatus status) {
        switch (status) {
    ...
    inline void record_audio_packet_ok(AudioReceiveStats& stats) {
        ++stats.packets_ok;
    }

    inline void record_audio_packet_lost(AudioReceiveStats& stats) {
        ++stats.packets_lost;
    }

    inline void record_audio_packet_rejected(AudioReceiveStats& stats) {
        ++stats.packets_rejected;
    }

    [[nodiscard]] inline Error record_audio_block_result(
    ...
    inline void reset_audio_receive_stats(AudioReceiveStats& stats) {
        stats = {};
    }
}
