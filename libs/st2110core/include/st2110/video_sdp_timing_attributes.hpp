...
parse_video_sdp_timing_attributes(const RawVideoSdpMediaSection &raw) {
  ...
  if (ts_refclk.has_value()) {
    ...
    res.reference_clock =
        RawVideoSdpScopedTimingValue<RawVideoSdpReferenceClock>{.value = std::move(*parsed), .scope = ts_refclk->scope};
  }

  const auto mediaclk = resolve_raw_sdp_scoped_timing_attribute(raw.session_mediaclk, raw.media_mediaclk, raw.mediaclk);
