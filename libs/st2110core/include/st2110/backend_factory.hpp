...
namespace st2110
{
    ...
    [[nodiscard]] inline std::expected<RxBackendKind, Error>
    parse_rx_backend_kind(std::string_view value) noexcept
    {
        if (value == "socket")
        {
    ...
    validate_rx_backend_descriptor(const RxBackendDescriptor& descriptor) noexcept
    {
    ...
    [[nodiscard]] inline Error
    validate_rx_backend_selection(const RxBackendSelection& selection) noexcept
    {
    ...
    select_rx_backend_factory(std::span<IRxBackendFactory* const> factories,
    ...
    create_rx_backend(std::span<IRxBackendFactory* const> factories,
                      const RxBackendSelection& selection)
    {
