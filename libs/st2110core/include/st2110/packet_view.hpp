...
namespace st2110 {
    ...
    [[nodiscard]] inline std::expected<PacketView, PacketViewParseFailure> parse_packet_view_staged(ByteSpan udp_payload) {
        ...
        std::expected<St2110PayloadHeaderView, Error> st2110_20_payload_header = parse_st2110_20_payload_header(rtp_payload);
        if (!st2110_20_payload_header.has_value()) {
            return std::unexpected(PacketViewParseFailure{st2110_20_payload_header.error(), PacketParseStage::St2110PayloadHeaderParse});
        }
