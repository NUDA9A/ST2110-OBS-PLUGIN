...
namespace st2110 {
    ...
    [[nodiscard]] inline Error validate_video_sdp_rtpmap_for_video_signaling(
            const RawVideoSdpRtpMap& rtpmap
    ) {
    ...
    [[nodiscard]] std::expected<std::array<uint8_t, N>, Error>
    parse_separated_hex_octets(std::string_view text) {
    ...
    [[nodiscard]] inline Error apply_video_sdp_timing_attributes_to_signaling(
            const RawVideoSdpTimingAttributes& raw,
    ...
    video_stream_signaling_from_raw_video_sdp_media_section(const RawVideoSdpMediaSection& raw) {
        ...
        auto expected_stream_signaling =
                video_stream_signaling_from_raw_video_sdp_fmtp(*expected_fmtp);
        ...
        if (Error err = validate_video_sdp_rtpmap_for_video_signaling(*expected_rtpmap);
                err != Error::Ok) {
    ...
    parse_video_stream_signaling_from_sdp(
        ...
        auto expected_raw_sdp_media_section =
                select_raw_video_sdp_media_section(sdp, expected_payload_type);

        if (!expected_raw_sdp_media_section.has_value()) {
            return std::unexpected(expected_raw_sdp_media_section.error());
        }

        return video_stream_signaling_from_raw_video_sdp_media_section(
                *expected_raw_sdp_media_section
        );
    }
}
