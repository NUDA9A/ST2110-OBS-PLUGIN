...
static VideoStreamSignaling make_valid_signaling() {
    ...
    signaling.media.signal_standard =
            VideoSignalStandard{VideoSignalStandard::Known::St2110_20_2022, std::nullopt};
...
static void test_known_sampling_rejects_raw_token() {
    const VideoSampling sampling{
            VideoSampling::Known::YCbCr420,
            std::string{"should-not-be-present"}};
    assert(validate_video_sampling(sampling) == Error::InvalidValue);
}
...
static void test_token_backed_video_media_fields_validate_known_and_other_cases() {
...

    {
        const VideoTransferCharacteristicSystem tcs{
                VideoTransferCharacteristicSystem::Known::PQ,
                std::nullopt};
        assert(validate_video_transfer_characteristic_system(tcs) == Error::Ok);
    }
    {
        const VideoTransferCharacteristicSystem tcs{
                VideoTransferCharacteristicSystem::Known::Other,
    ...
    {
        const VideoSignalStandard ssn{
                VideoSignalStandard::Known::St2110_20_2022,
                std::nullopt};
        assert(validate_video_signal_standard(ssn) == Error::Ok);
    }
    {
        const VideoSignalStandard ssn{
                VideoSignalStandard::Known::Other,
                std::string{"ST2110-20:2099"}};
        assert(validate_video_signal_standard(ssn) == Error::Ok);
    }
    {
        const VideoSignalStandard ssn{
