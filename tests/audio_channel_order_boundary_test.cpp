...
namespace {
st2110::AudioStreamSignaling make_level_a_stream(uint16_t channel_count, std::string channel_order = {}) {
    ...
    if (!channel_order.empty()) {
        signaling.channel_order = st2110::AudioChannelOrderSignaling{
            .convention = st2110::AudioChannelOrderConvention::Smpte2110, .raw_value = std::move(channel_order)};
    }
...
void parses_smpte2110_surround_and_stereo_groups() {
    const st2110::AudioChannelOrderSignaling channel_order{.convention = st2110::AudioChannelOrderConvention::Smpte2110,
...
void rejects_malformed_smpte2110_channel_order_values() {
    const auto assert_invalid = [](std::string raw_value) {
        const st2110::AudioChannelOrderSignaling channel_order{
            .convention = st2110::AudioChannelOrderConvention::Smpte2110, .raw_value = std::move(raw_value)};
...
void absent_channel_order_becomes_effective_undefined_group() {
    ...
    const auto effective = st2110::effective_audio_channel_order_from_audio_stream_signaling(signaling);
...
void non_smpte2110_other_convention_is_preserved_but_not_mapped_to_layout() {
    const st2110::AudioChannelOrderSignaling channel_order{.convention = st2110::AudioChannelOrderConvention::Other,
    ...
    assert(st2110::validate_audio_channel_order_against_channel_count(channel_order, 2) == st2110::Error::Ok);
}
} // namespace
