...
void bootstrap_from_absent_channel_order_derives_rx_config_and_undefined_group() {
    ...
    auto bootstrap = audio_receiver_bootstrap_config_from_audio_stream_signaling(signaling, 5004, 112, "", "239.1.1.1");
...
void bootstrap_preserves_exact_smpte2110_channel_order() {
    ...
    signaling.channel_order =
        AudioChannelOrderSignaling{.convention = AudioChannelOrderConvention::Smpte2110, .raw_value = "SMPTE2110.(ST)"};
...
void bootstrap_appends_undefined_group_for_under_declared_smpte2110_order() {
    ...
    signaling.channel_order =
        AudioChannelOrderSignaling{.convention = AudioChannelOrderConvention::Smpte2110, .raw_value = "SMPTE2110.(51)"};
...
void bootstrap_preserves_other_channel_order_as_unknown_convention() {
    auto signaling = make_level_a_audio_stream(2);
    signaling.channel_order = AudioChannelOrderSignaling{.convention = AudioChannelOrderConvention::Other,
...
void bootstrap_rejects_invalid_runtime_transport_fields() {
    ...
    auto bad_port = audio_receiver_bootstrap_config_from_audio_stream_signaling(signaling, 0, 112, "", "239.1.1.1");
    assert(!bad_port.has_value());
    assert(bad_port.error() == Error::InvalidValue);

    auto bad_payload_type =
        audio_receiver_bootstrap_config_from_audio_stream_signaling(signaling, 5004, 95, "", "239.1.1.1");
    assert(!bad_payload_type.has_value());
    assert(bad_payload_type.error() == Error::InvalidValue);

    auto bad_dest_ip = audio_receiver_bootstrap_config_from_audio_stream_signaling(signaling, 5004, 112, "", "");
