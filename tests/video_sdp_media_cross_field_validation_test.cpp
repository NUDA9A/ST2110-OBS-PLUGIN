...
static void assert_runtime_projection_is_unsupported(
        const st2110::VideoStreamSignaling& signaling
) {
...
static void test_progressive_420_sampling_is_structurally_valid_but_runtime_unsupported() {
    const std::array<st2110::VideoSampling::Known, 3> variants{
            st2110::VideoSampling::Known::YCbCr420,
    ...
    for (const auto sampling : variants) {
        {
            ...
            assert(st2110::validate_video_stream_signaling(signaling) ==
...
static std::string make_sdp_with_fmtp(std::string_view fmtp_payload) {
    return std::string{
            ...
            "a=fmtp:112 "} + std::string{fmtp_payload} + "\n";
}

static void test_sdp_ingestion_applies_420_cross_field_validation() {
    const std::string progressive_420_sdp = make_sdp_with_fmtp(
            "sampling=YCbCr-4:2:0; width=1920; height=1080; exactframerate=25; "
            "depth=8; colorimetry=BT709; PM=2110GPM; SSN=ST2110-20:2022"
    );

    auto progressive = st2110::parse_video_stream_signaling_from_sdp(
            progressive_420_sdp,
            112
    );
    ...
    const std::string interlaced_420_sdp = make_sdp_with_fmtp(
            "sampling=YCbCr-4:2:0; width=1920; height=1080; exactframerate=25; "
            "depth=8; colorimetry=BT709; PM=2110GPM; SSN=ST2110-20:2022; interlace"
    );

    auto interlaced = st2110::parse_video_stream_signaling_from_sdp(
            interlaced_420_sdp,
            112
    );
...
static void test_sdp_ingestion_applies_key_cross_field_validation() {
    const std::string valid_key_sdp = make_sdp_with_fmtp(
            "sampling=KEY; width=1920; height=1080; exactframerate=25; "
            "depth=8; colorimetry=ALPHA; PM=2110GPM; SSN=ST2110-20:2022"
    );

    auto valid_key = st2110::parse_video_stream_signaling_from_sdp(
            valid_key_sdp,
            112
    );
    ...
    const std::string non_alpha_key_sdp = make_sdp_with_fmtp(
            "sampling=KEY; width=1920; height=1080; exactframerate=25; "
            "depth=8; colorimetry=BT709; PM=2110GPM; SSN=ST2110-20:2022"
    );

    auto non_alpha_key = st2110::parse_video_stream_signaling_from_sdp(
            non_alpha_key_sdp,
            112
    );
    ...
    const std::string key_with_tcs_sdp = make_sdp_with_fmtp(
            "sampling=KEY; width=1920; height=1080; exactframerate=25; "
            "depth=8; colorimetry=ALPHA; TCS=SDR; PM=2110GPM; SSN=ST2110-20:2022"
    );

    auto key_with_tcs = st2110::parse_video_stream_signaling_from_sdp(
            key_with_tcs_sdp,
            112
    );
