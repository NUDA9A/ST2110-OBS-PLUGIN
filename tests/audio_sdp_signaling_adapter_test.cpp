...
#include <st2110/audio_sdp_signaling_adapter.hpp>
...
int main() {
    ...
    static_assert(std::is_same_v<
                  decltype(audio_stream_signaling_from_raw_audio_sdp_media_section(
...

    {
        for (const std::uint16_t channel_count : {std::uint16_t{1}, std::uint16_t{8}}) {
            const std::string sdp =
                    "v=0\r\n"
                    "m=audio 5004 RTP/AVP 111\r\n"
                    "a=rtpmap:111 L24/48000/" + std::to_string(channel_count) + "\r\n"
                                                                                "a=ptime:1\r\n";
