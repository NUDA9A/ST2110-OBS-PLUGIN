...
namespace {
    ...
    std::string valid_session_refclk(unsigned domain = 1) {
        return "a=ts-refclk:ptp=IEEE1588-2008:00-11-22-33-44-55-66-77:" +
    ...
    std::string make_video_sdp(
            const std::string& session_attributes = {},
            const std::string& media_attributes = {},
    ...
    void session_level_reference_clock_and_media_clock_are_preserved_and_applied() {
        const std::string sdp = make_video_sdp(
                valid_session_refclk(1) +
                "a=mediaclk:direct=0\n"
        );
        ...
        assert(raw->session_ts_refclk->value ==
               "ptp=IEEE1588-2008:00-11-22-33-44-55-66-77:1");

        assert(!raw->media_ts_refclk.has_value());
        assert(raw->ts_refclk.has_value());
        assert(*raw->ts_refclk ==
               "ptp=IEEE1588-2008:00-11-22-33-44-55-66-77:1");
    ...
    void media_level_reference_clock_and_media_clock_override_session_level_values() {
        ...
        assert(raw->media_ts_refclk->value ==
               "ptp=IEEE1588-2008:00-11-22-33-44-55-66-77:2");
    ...
    void existing_media_level_only_sdp_behavior_remains_unchanged() {
        const std::string sdp = make_video_sdp(
                {},
                "a=ts-refclk:ptp=IEEE1588-2008:00-11-22-33-44-55-66-77:3\n"
                "a=mediaclk:direct=0\n"
                "a=tsmode:NEW\n"
                "a=tsdelay:1800\n"
                "a=TP:2110TPW\n"
