...
namespace {
    ...
    bool has_attribute(
            const std::vector<st2110::RawAudioSdpAttribute>& attributes,
    ...
    void valid_level_a_sdp_maps_to_audio_stream_signaling() {
        auto parsed = st2110::parse_audio_stream_signaling_from_sdp(
                valid_level_a_sdp_with_channel_order(),
                kPayloadType);
    ...
    void missing_media_level_mediaclk_is_rejected_by_final_ingestion() {
        auto parsed = st2110::parse_audio_stream_signaling_from_sdp(
                sdp_missing_media_level_mediaclk(),
                kPayloadType);
    ...
    void session_level_only_mediaclk_is_not_enough_for_final_ingestion() {
        auto parsed = st2110::parse_audio_stream_signaling_from_sdp(
                sdp_with_session_level_only_mediaclk(),
                kPayloadType);
    ...
    void unknown_attributes_are_preserved_raw_but_ignored_by_final_mapping() {
        ...
        assert(has_attribute(
                raw->unknown_session_attributes,
                "x-session-unknown",
                "preserve-session"));
        assert(has_attribute(
                raw->unknown_attributes,
                "x-media-unknown",
                "preserve-media"));
