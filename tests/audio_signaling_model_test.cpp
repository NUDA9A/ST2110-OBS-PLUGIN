...
int main() {
    ...
    static_assert(
        std::is_same_v<decltype(validate_audio_conformance_range(std::declval<const AudioConformanceRange &>())),
                       Error>);

    static_assert(std::is_same_v<decltype(audio_media_description_matches_conformance_range(
                                     std::declval<const AudioMediaDescription &>(),
                                     std::declval<const AudioConformanceRange &>())),
                                 bool>);

    static_assert(std::is_same_v<decltype(validate_audio_media_description_against_conformance_range(
                                     std::declval<const AudioMediaDescription &>(),
                                     std::declval<const AudioConformanceRange &>())),
                                 Error>);

    static_assert(std::is_same_v<decltype(validate_audio_channel_order_signaling(
