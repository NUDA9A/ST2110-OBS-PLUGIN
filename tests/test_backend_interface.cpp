...
static_assert(std::is_same_v<
    decltype(st2110::supports_media(
        std::declval<const st2110::RxBackendCapabilities&>(),
...
static_assert(std::is_same_v<
    decltype(std::declval<const st2110::IRxBackend&>().capabilities()),
    st2110::RxBackendCapabilities>);
...
static_assert(std::is_same_v<
    decltype(std::declval<st2110::IRxVideoBackend&>().start_video(
        std::declval<const st2110::RxVideoConfig&>(),
        std::declval<st2110::IVideoFrameSink&>())),
    void>);

static_assert(std::is_same_v<
    decltype(std::declval<st2110::IRxAudioBackend&>().start_audio(
...
namespace
{
    ...
    public:
        ...
        void start_video(const st2110::RxVideoConfig& cfg,
    ...
    public:
        ...
        void start_audio(const st2110::RxAudioConfig& cfg,
    ...
    class FakeCombinedBackend final :
        public st2110::IRxVideoBackend,
        public st2110::IRxAudioBackend
    {
    public:
        const char* backend_name() const override
        {
    ...
    void test_backend_capability_helper()
    {
        ...
        assert(!st2110::supports_media(
            combined,
            static_cast<st2110::RxMediaKind>(255)));
    }

    void test_fake_video_backend_delivers_video_frame()
    {
        ...
        const st2110::IRxBackend& backend_base = backend;
        const auto capabilities = backend_base.capabilities();

        assert(std::string_view(backend_base.backend_name()) == "fake-video");
    ...
    void test_fake_audio_backend_delivers_audio_frame()
    {
        ...
        assert(std::string_view(backend_base.backend_name()) == "fake-audio");
        assert(!st2110::supports_media(capabilities, st2110::RxMediaKind::Video));
        assert(st2110::supports_media(capabilities, st2110::RxMediaKind::Audio));

        backend.start_audio(cfg, sink);
    ...
    void test_combined_backend_has_single_common_backend_base()
    {
        ...
        assert(!backend.video_started);
        assert(!backend.audio_started);
        assert(!backend.stopped);

        video_backend.start_video(video_cfg, video_sink);
        audio_backend.start_audio(audio_cfg, audio_sink);
...
int main()
{
    test_backend_capability_helper();
    test_fake_video_backend_delivers_video_frame();
    test_fake_audio_backend_delivers_audio_frame();
    test_combined_backend_has_single_common_backend_base();
    return 0;
}
