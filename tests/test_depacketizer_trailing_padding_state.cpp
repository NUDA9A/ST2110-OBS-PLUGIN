...
static std::vector<uint8_t> make_packet(uint16_t seq,
    ...
    p.push_back(0x00); p.push_back(0x00); // ext seq hi16

    p.push_back(static_cast<uint8_t>((srd_length >> 8) & 0xFF));
    p.push_back(static_cast<uint8_t>(srd_length & 0xFF));

    p.push_back(0x00); p.push_back(0x00); // F=0, row=0
    p.push_back(0x00); p.push_back(0x00); // C=0, offset=0
...
static st2110::PacketView parse_packet(uint16_t seq,
                                       uint32_t rtp_timestamp,
                                       bool marker,
                                       const std::vector<uint8_t>& segment_bytes,
                                       const std::vector<uint8_t>& trailing_bytes) {
...
static void test_invalid_first_packet_does_not_open_unit() {
    ...
    const st2110::PacketView pkt =
            parse_packet(1, 100, false, {1, 2, 3, 4}, {0, 0});
...
static void test_invalid_key_transition_packet_does_not_close_previous_unit() {
    ...
    {
        const st2110::PacketView first =
                parse_packet(1, 100, false, {1, 2, 3, 4}, {});
    ...
    {
        const st2110::PacketView bad_transition =
                parse_packet(2, 200, false, {5, 6, 7, 8}, {0, 0});
    ...
    {
        const st2110::PacketView final_pkt =
                parse_packet(3, 100, true, {1, 2, 3, 4}, {});
