...
static_assert(std::is_same_v<
        decltype(st2110::config_validation::is_non_empty("x")),
        bool>);

static_assert(std::is_same_v<
        decltype(st2110::config_validation::is_dynamic_rtp_payload_type(static_cast<uint8_t>(96))),
        bool>);
...
static void test_video_format_constraints_for_uyvy() {
    assert(st2110::config_validation::validate_video_format_constraints(
            st2110::PixelFormat::UYVY, 1920, 1080) == st2110::Error::Ok);

    assert(st2110::config_validation::validate_video_format_constraints(
            st2110::PixelFormat::UYVY, 1919, 1080) == st2110::Error::InvalidValue);

    assert(st2110::config_validation::validate_video_format_constraints(
            st2110::PixelFormat::UYVY, 1920, 0) == st2110::Error::InvalidValue);
}

static void test_unknown_format_is_unsupported() {
    const auto unknown = static_cast<st2110::PixelFormat>(999);
    assert(st2110::config_validation::validate_video_format_constraints(
            unknown, 1920, 1080) == st2110::Error::Unsupported);
}
