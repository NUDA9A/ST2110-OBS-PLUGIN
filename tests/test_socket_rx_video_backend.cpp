...
namespace
{
    ...
    {
    public:
        explicit FakeSocketRxPort(std::shared_ptr<FakeSocketRxPortState> state)
    ...
    {
    public:
        explicit FakeSocketRxPortFactory(std::shared_ptr<FakeSocketRxPortState> state)
            : state_(std::move(state))
        {
        }
    ...
    void test_socket_rx_video_backend_uses_injected_socket_port_factory_for_ipv4_projection()
    {
        ...
        st2110::SocketRxVideoBackend backend(
            std::make_unique<FakeSocketRxPortFactory>(state));
