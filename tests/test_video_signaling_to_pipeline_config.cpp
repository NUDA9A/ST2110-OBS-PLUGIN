...
static void test_invalid_signaling_is_rejected_before_runtime_projection() {
    ...
    auto dep_cfg = st2110::depacketizer_config_from_video_stream_signaling(
            s,
            st2110::PartialFramePolicy::EmitWithFlag);
    ...
    auto pipe_cfg = st2110::video_receive_pipeline_config_from_video_stream_signaling(
            s,
            st2110::PartialFramePolicy::EmitWithFlag);
...
static void test_structurally_valid_but_unmappable_runtime_format_is_unsupported() {
    st2110::VideoStreamSignaling s = make_base_signaling();
    s.media.depth = st2110::VideoBitDepth{10, false}; // structurally valid, but not currently mappable to UYVY runtime path
