...
static void test_validate_media_clock_mode_accepts_direct() {
    assert(st2110::validate_media_clock_mode(st2110::MediaClockMode::Direct)
           == st2110::Error::Ok);
}

static void test_validate_media_clock_mode_accepts_sender() {
    assert(st2110::validate_media_clock_mode(st2110::MediaClockMode::Sender)
           == st2110::Error::Ok);
}
...
static void test_validate_media_clock_mode_rejects_invalid_enum() {
    ...
    assert(st2110::validate_media_clock_mode(invalid)
           == st2110::Error::InvalidValue);
}

static void test_validate_timestamp_mode_accepts_samp() {
    assert(st2110::validate_timestamp_mode(st2110::TimestampMode::Samp)
           == st2110::Error::Ok);
}

static void test_validate_timestamp_mode_accepts_new() {
    assert(st2110::validate_timestamp_mode(st2110::TimestampMode::New)
           == st2110::Error::Ok);
}

static void test_validate_timestamp_mode_accepts_pres() {
    assert(st2110::validate_timestamp_mode(st2110::TimestampMode::Pres)
           == st2110::Error::Ok);
}
...
static void test_validate_timestamp_mode_rejects_invalid_enum() {
    ...
    assert(st2110::validate_timestamp_mode(invalid)
           == st2110::Error::InvalidValue);
}

static void test_validate_video_timing_signaling_accepts_direct_new_zero_delay() {
    assert(st2110::validate_video_timing_signaling(
            st2110::MediaClockMode::Direct,
            st2110::TimestampMode::New,
            0)
           == st2110::Error::Ok);
}
...
static void test_validate_video_timing_signaling_accepts_sender_pres_nonzero_delay() {
    assert(st2110::validate_video_timing_signaling(
            st2110::MediaClockMode::Sender,
            st2110::TimestampMode::Pres,
            1234)
           == st2110::Error::Ok);
}
...
static void test_video_stream_signaling_rejects_invalid_media_clock_mode() {
    ...
    assert(st2110::validate_video_stream_signaling(s)
           == st2110::Error::InvalidValue);
}
