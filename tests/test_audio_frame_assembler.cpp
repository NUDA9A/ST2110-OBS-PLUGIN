...
namespace {
    ...
    AudioRtpPacketView make_packet(
            uint16_t seq,
            uint32_t timestamp,
            bool marker,
    ...
    void rejects_payload_size_mismatch_without_emitting_block() {
        std::vector<uint8_t> payload{
                ...
                0x00, 0x00, 0x02,
                0x00, 0x00, 0x03,
                0x00, 0x00       // one byte short for 2 channels * 2 samples * L24
        };
    ...
    void rejects_invalid_packet_shape() {
...

        {
            ...
            auto packet = make_packet(
                    ...
                    1,
                    1,
                    static_cast<AudioPcmWireFormat>(99));
    ...
    void reset_clears_stats_only_and_keeps_assembler_reusable() {
        ...
        const auto bad_packet = make_packet(
                107,
                123,
                false,
                bad_payload,
        ...
        std::vector<uint8_t> good_payload{
                0x00, 0x2A
        };

        const auto good_packet = make_packet(
                108,
                456,
                false,
                good_payload,
