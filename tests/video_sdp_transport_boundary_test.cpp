...
static constexpr std::string_view kBaseFmtp =
...
static std::string make_valid_sdp_with_transport() {
    return std::string{
            ...
            "a=fmtp:112 "} + std::string{kBaseFmtp} + "\n"
                                                      "a=media-future:kept\n";
}
...
static void test_transport_metadata_does_not_break_video_signaling_ingestion() {
    auto signaling = st2110::parse_video_stream_signaling_from_sdp(
            make_valid_sdp_with_transport(),
            112
    );
