...
static_assert(std::is_same_v<decltype(std::declval<st2110::LinuxSocketRxPort&>().open(
                             ...
                             st2110::Error>);
static_assert(std::is_same_v<decltype(std::declval<st2110::LinuxSocketRxPort&>().close()),
                             st2110::Error>);
...
namespace
{
    ...
    public:
        ...
        ReservedUdpSocket(int fd, uint16_t port)
            : fd_(fd), port_(port)
        {
        }
    ...
    st2110::SocketRxOpenConfig make_ipv4_multicast_open_config(uint16_t port)
    {
        st2110::SocketRxOpenConfig cfg{};
        cfg.bind_endpoint.family = st2110::SocketAddressFamily::IPv4;
    ...
    void test_linux_socket_rx_port_rejects_invalid_or_unsupported_open_requests()
    {
        ...
        const auto multicast_cfg = make_ipv4_multicast_open_config(5004);
        assert(port_obj.open(multicast_cfg) == st2110::Error::Unsupported);
        assert(!port_obj.is_open());
    }
