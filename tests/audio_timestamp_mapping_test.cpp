...
void rejects_tick_to_ns_overflow() {
  const auto too_large = audio_rtp_ticks_to_timestamp_ns(std::numeric_limits<uint64_t>::max(), 48'000);
...
void rejects_backward_or_invalid_mapping() {
  {
    AudioRtpTimestampMapper mapper({.rtp_clock_rate = 48'000, .anchor_rtp_timestamp = 1000, .anchor_timestamp_ns = 0});
...
void computes_receiver_playout_timing_decision() {
  const AudioReceiverPlayoutTimingConfig cfg{.playout_delay_ns = 250'000};
...
void rejects_receiver_playout_timing_overflow() {
  const AudioReceiverPlayoutTimingConfig cfg{.playout_delay_ns = 100};

  const auto decision = audio_receiver_playout_timing_decision(std::numeric_limits<TimestampNs>::max() - 50, cfg);
...
void computes_audio_block_timing_without_coupling_to_assembler() {
  const AudioReceiverPlayoutTimingConfig cfg{.playout_delay_ns = 500'000};

  const auto timing = audio_block_timing(12345, 2'000'000, cfg);
