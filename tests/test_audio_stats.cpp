...
static void block_status_routes_to_the_matching_counter() {
  ...
  assert(record_audio_block_result(stats, AudioBlockCompletionStatus::Complete) == Error::Ok);
  assert(record_audio_block_result(stats, AudioBlockCompletionStatus::Partial) == Error::Ok);
  assert(record_audio_block_result(stats, AudioBlockCompletionStatus::Partial) == Error::Ok);
  assert(record_audio_block_result(stats, AudioBlockCompletionStatus::Dropped) == Error::Ok);
...
static void invalid_block_status_is_rejected_without_mutation() {
  ...
  const auto invalid_status = static_cast<AudioBlockCompletionStatus>(255);

  assert(validate_audio_block_completion_status(invalid_status) == Error::InvalidValue);

  assert(record_audio_block_result(stats, invalid_status) == Error::InvalidValue);
