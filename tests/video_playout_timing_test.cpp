...
static void computes_reconstruction_time_from_mapped_media_timestamp() {
  const VideoReceiverPlayoutTimingConfig cfg{.link_offset_delay_ns = 5'000'000};

  auto decision = video_receiver_playout_timing_decision(1'000'000'000ULL, cfg);
...
static void rejects_reconstruction_timestamp_overflow() {
  const VideoReceiverPlayoutTimingConfig cfg{.link_offset_delay_ns = 1};

  auto decision = video_receiver_playout_timing_decision(std::numeric_limits<TimestampNs>::max(), cfg);
...
static void attaches_playout_timing_to_reconstructed_frame_metadata() {
  ReconstructedVideoFrame frame{VideoFrame(2, 2, PixelFormat::UYVY), 1234U, true};

  const VideoReceiverPlayoutTimingConfig cfg{.link_offset_delay_ns = 100};

  auto timing = video_reconstructed_frame_timing(frame, 50'000, cfg);
