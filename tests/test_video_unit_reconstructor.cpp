...
static st2110::AssembledVideoUnit make_unit(
        st2110::VideoAssemblyUnitKind kind,
    ...
    st2110::AssembledVideoUnit unit{
...
static void test_factory_returns_progressive_reconstructor() {
    auto reconstructor = st2110::make_video_unit_reconstructor(
            make_cfg(st2110::VideoScanMode::Progressive));
...
static void test_factory_rejects_interlaced_for_now() {
    auto reconstructor = st2110::make_video_unit_reconstructor(
            make_cfg(st2110::VideoScanMode::Interlaced));
...
static void test_factory_rejects_psf_for_now() {
    auto reconstructor = st2110::make_video_unit_reconstructor(
            make_cfg(st2110::VideoScanMode::PsF));
...
static void test_progressive_reconstructor_passes_through_complete_frame_unit() {
    auto reconstructor_expected = st2110::make_video_unit_reconstructor(
            make_cfg(st2110::VideoScanMode::Progressive));
    assert(reconstructor_expected.has_value());

    static const uint8_t bytes[] = {0,1,2,3,4,5,6,7};
    auto unit = make_unit(
            st2110::VideoAssemblyUnitKind::Frame,
            1000,
            true,
            bytes,
            sizeof(bytes));
...
static void test_progressive_reconstructor_rejects_wrong_unit_kind() {
    ...
    auto unit = make_unit(
            st2110::VideoAssemblyUnitKind::Field,
            3000,
            true,
            bytes,
            sizeof(bytes));
