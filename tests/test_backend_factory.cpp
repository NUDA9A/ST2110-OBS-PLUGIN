...
static_assert(std::is_same_v<
    decltype(st2110::validate_rx_backend_kind(
        st2110::RxBackendKind::Socket)),
    st2110::Error>);

static_assert(std::is_same_v<
    decltype(st2110::rx_backend_kind_name(
        st2110::RxBackendKind::Socket)),
    std::string_view>);

static_assert(std::is_same_v<
    decltype(st2110::parse_rx_backend_kind(
        std::declval<std::string_view>())),
    std::expected<st2110::RxBackendKind, st2110::Error>>);

static_assert(std::is_same_v<
    decltype(st2110::validate_rx_backend_descriptor(
        std::declval<const st2110::RxBackendDescriptor&>())),
    st2110::Error>);

static_assert(std::is_same_v<
    decltype(st2110::validate_rx_backend_selection(
...
static_assert(std::is_same_v<
    decltype(std::declval<const st2110::IRxBackendFactory&>().descriptor()),
    st2110::RxBackendDescriptor>);

static_assert(std::is_same_v<
    decltype(std::declval<const st2110::IRxBackendFactory&>().create_backend()),
    std::unique_ptr<st2110::IRxBackend>>);

static_assert(std::is_same_v<
    decltype(st2110::select_rx_backend_factory(
        std::declval<std::span<st2110::IRxBackendFactory* const>>(),
        std::declval<const st2110::RxBackendSelection&>())),
    std::expected<st2110::IRxBackendFactory*, st2110::Error>>);

static_assert(std::is_same_v<
    decltype(st2110::create_rx_backend(
        std::declval<std::span<st2110::IRxBackendFactory* const>>(),
        std::declval<const st2110::RxBackendSelection&>())),
...
namespace
{
    ...
    {
    public:
        FakeBackend(std::string_view backend_name,
                    st2110::RxBackendCapabilities backend_capabilities)
    ...
    {
    public:
        explicit FakeBackendFactory(st2110::RxBackendDescriptor descriptor,
                                    bool return_null_backend = false)
            : descriptor_(descriptor),
              return_null_backend_(return_null_backend)
        {
        }

        st2110::RxBackendDescriptor descriptor() const override
        {
            return descriptor_;
        }
        ...
        std::unique_ptr<st2110::IRxBackend> create_backend() const override
        {
            ...
            return std::make_unique<FakeBackend>(descriptor_.name,
    ...
    {
        assert(st2110::validate_rx_backend_kind(st2110::RxBackendKind::Socket) ==
            st2110::Error::Ok);
        assert(st2110::validate_rx_backend_kind(st2110::RxBackendKind::Mtl) ==
            st2110::Error::Ok);
        assert(st2110::validate_rx_backend_kind(
                static_cast<st2110::RxBackendKind>(255)) ==
            st2110::Error::InvalidValue);

        assert(st2110::rx_backend_kind_name(st2110::RxBackendKind::Socket) ==
            std::string_view{"socket"});
        assert(st2110::rx_backend_kind_name(st2110::RxBackendKind::Mtl) ==
            std::string_view{"mtl"});
        assert(st2110::rx_backend_kind_name(
            static_cast<st2110::RxBackendKind>(255)).empty());
    ...
    {
        st2110::RxBackendDescriptor valid_socket{
            st2110::RxBackendKind::Socket,
            "socket",
            combined_capabilities(),
            true
        };
        assert(st2110::validate_rx_backend_descriptor(valid_socket) ==
            st2110::Error::Ok);

        st2110::RxBackendDescriptor unavailable_mtl{
            st2110::RxBackendKind::Mtl,
            "mtl",
            video_only_capabilities(),
            false
        };
        assert(st2110::validate_rx_backend_descriptor(unavailable_mtl) ==
            st2110::Error::Ok);

        st2110::RxBackendDescriptor invalid_kind = valid_socket;
        invalid_kind.kind = static_cast<st2110::RxBackendKind>(255);
        assert(st2110::validate_rx_backend_descriptor(invalid_kind) ==
            st2110::Error::InvalidValue);

        st2110::RxBackendDescriptor empty_name = valid_socket;
        empty_name.name = {};
        assert(st2110::validate_rx_backend_descriptor(empty_name) ==
            st2110::Error::InvalidValue);

        st2110::RxBackendDescriptor no_media = valid_socket;
        no_media.capabilities = {};
        assert(st2110::validate_rx_backend_descriptor(no_media) ==
            st2110::Error::InvalidValue);
    }

    void test_selection_validation()
    {
        st2110::RxBackendSelection valid{
            st2110::RxBackendKind::Socket,
            st2110::RxMediaKind::Video
        };
        assert(st2110::validate_rx_backend_selection(valid) ==
            st2110::Error::Ok);

        st2110::RxBackendSelection invalid_backend_kind = valid;
        invalid_backend_kind.backend_kind =
            static_cast<st2110::RxBackendKind>(255);
        assert(st2110::validate_rx_backend_selection(invalid_backend_kind) ==
            st2110::Error::InvalidValue);

        st2110::RxBackendSelection invalid_media_kind = valid;
        invalid_media_kind.media_kind =
            static_cast<st2110::RxMediaKind>(255);
        assert(st2110::validate_rx_backend_selection(invalid_media_kind) ==
            st2110::Error::InvalidValue);
    }
    ...
    void test_selector_picks_requested_backend_and_media()
    {
        ...
        std::array<st2110::IRxBackendFactory*, 2> factories{
            &socket_factory,
            &mtl_factory
        };
    ...
    void test_selector_rejects_unavailable_backend()
    {
        ...
        std::array<st2110::IRxBackendFactory*, 1> factories{
            &unavailable_mtl_factory
        };
    ...
    void test_create_backend_uses_selected_factory()
    {
        ...
        assert(st2110::supports_media((*backend)->capabilities(),
            st2110::RxMediaKind::Video));
        assert(!st2110::supports_media((*backend)->capabilities(),
            st2110::RxMediaKind::Audio));
