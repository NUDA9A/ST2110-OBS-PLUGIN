...
static_assert(
    std::is_same_v<decltype(st2110::validate_socket_address_family(st2110::SocketAddressFamily::IPv4)), st2110::Error>);

static_assert(
    std::is_same_v<decltype(st2110::socket_address_family_name(st2110::SocketAddressFamily::IPv4)), std::string_view>);

static_assert(std::is_same_v<decltype(st2110::is_ipv4_multicast_address(std::declval<std::string_view>())), bool>);

static_assert(std::is_same_v<decltype(st2110::is_ipv6_multicast_address(std::declval<std::string_view>())), bool>);

static_assert(std::is_same_v<decltype(st2110::validate_socket_endpoint(std::declval<const st2110::SocketEndpoint &>())),
                             st2110::Error>);
...
static_assert(
    std::is_same_v<decltype(st2110::validate_socket_rx_open_config(std::declval<const st2110::SocketRxOpenConfig &>())),
                   st2110::Error>);

static_assert(std::is_same_v<
              decltype(st2110::socket_rx_uses_multicast(std::declval<const st2110::SocketRxOpenConfig &>())), bool>);

static_assert(std::is_same_v<
              decltype(st2110::socket_rx_open_config_from_video_config(std::declval<const st2110::RxVideoConfig &>())),
...
static_assert(std::is_same_v<decltype(std::declval<st2110::ISocketRxPort &>().open(
                             ...
                             st2110::Error>);
static_assert(std::is_same_v<decltype(std::declval<st2110::ISocketRxPort &>().close()), st2110::Error>);
static_assert(
    std::is_same_v<decltype(std::declval<st2110::ISocketRxPort &>().receive(std::declval<std::span<std::uint8_t>>())),
...
class FakeSocketRxPort final : public st2110::ISocketRxPort {
  public:
    ...
    std::expected<st2110::SocketReceiveResult, st2110::Error> receive(std::span<std::uint8_t> buffer) override {
        if (!open_) {
...
void test_socket_open_config_validation_and_multicast_flag() {
    ...
    cfg4.multicast_membership = st2110::SocketMulticastMembership{
        .family = st2110::SocketAddressFamily::IPv4, .group_address = "239.1.2.3", .interface_address = ""};
