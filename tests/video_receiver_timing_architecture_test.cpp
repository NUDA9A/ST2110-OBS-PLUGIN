...
using GenericBootstrapResult = decltype(video_receiver_bootstrap_config_from_video_stream_signaling(
    std::declval<const VideoStreamSignaling &>(), std::declval<uint16_t>(), std::declval<uint8_t>(),
    std::declval<std::string>(), std::declval<std::string>(), std::declval<PartialFramePolicy>()));

using TimingBootstrapResult = decltype(video_receiver_bootstrap_config_from_video_stream_signaling(
    std::declval<const VideoStreamSignaling &>(), std::declval<const VideoReceiverTimingConfig &>(),
    std::declval<uint16_t>(), std::declval<uint8_t>(), std::declval<std::string>(), std::declval<std::string>(),
...
void assert_same_runtime_projection_except_timing(const VideoReceiverBootstrapConfig &generic_cfg,
...
void assert_same_timing_config(const VideoReceiverTimingConfig &expected, const VideoReceiverTimingConfig &actual) {
...
void test_timing_bootstrap_reuses_generic_runtime_projection() {
    ...
    const auto generic_result = video_receiver_bootstrap_config_from_video_stream_signaling(
        signaling, 5004, 96, "127.0.0.1", "239.10.10.10", PartialFramePolicy::EmitWithFlag);

    assert(generic_result.has_value());

    const auto timing_result = video_receiver_bootstrap_config_from_video_stream_signaling(
        signaling, timing, 5004, 96, "127.0.0.1", "239.10.10.10", PartialFramePolicy::EmitWithFlag);
...
void test_ts_delay_rejection_is_localized_in_timing_boundary() {
    ...
    const auto generic_result = video_receiver_bootstrap_config_from_video_stream_signaling(
        signaling, 5004, 96, "127.0.0.1", "239.10.10.10", PartialFramePolicy::Drop);

    assert(generic_result.has_value());

    const auto timing_result = video_receiver_bootstrap_config_from_video_stream_signaling(
        signaling, timing, 5004, 96, "127.0.0.1", "239.10.10.10", PartialFramePolicy::Drop);
