...
static void test_parses_known_ptp_reference_clock() {
    auto parsed = parse_video_sdp_reference_clock(
            "ptp=IEEE1588-2008:00-11-22-33-44-55-66-77:127"
    );
...
static void test_parses_ptp_reference_clock_without_domain() {
    auto parsed = parse_video_sdp_reference_clock(
            "ptp=IEEE1588-2008:00-11-22-33-44-55-66-77"
    );
...
static void test_rejects_malformed_ptp_reference_clock_domain() {
    auto parsed = parse_video_sdp_reference_clock(
            "ptp=IEEE1588-2008:00-11-22-33-44-55-66-77:999"
    );
...
static void test_parses_aggregate_timing_attributes_from_raw_media_section() {
    ...
    assert(timing.reference_clock->kind == RawVideoSdpReferenceClock::Kind::Ptp);
    assert(timing.reference_clock->ptp.has_value());
    assert(timing.reference_clock->ptp->version == "IEEE1588-2008");
    assert(timing.reference_clock->ptp->gmid == "00-11-22-33-44-55-66-77");
    assert(timing.reference_clock->ptp->domain.has_value());
    assert(*timing.reference_clock->ptp->domain == 127);

    assert(timing.media_clock.has_value());
    assert(timing.media_clock->kind == RawVideoSdpMediaClock::Kind::Direct);
    assert(timing.media_clock->direct_offset.has_value());
    assert(*timing.media_clock->direct_offset == 0);

    assert(timing.timestamp_mode.has_value());
    assert(timing.timestamp_mode->raw_token == "NEW");

    assert(timing.ts_delay_sender_ticks.has_value());
    assert(*timing.ts_delay_sender_ticks == 1800);

    assert(timing.sender_type.has_value());
    assert(timing.sender_type->raw_token == "2110TPN");

    assert(timing.troff_us.has_value());
    assert(*timing.troff_us == 42);

    assert(timing.cmax.has_value());
    assert(*timing.cmax == 7);
}
...
static void test_aggregate_parser_propagates_error() {
    RawVideoSdpMediaSection raw{};
...
int main() {
    ...
    test_parses_aggregate_timing_attributes_from_raw_media_section();
    test_aggregate_parser_leaves_absent_fields_empty();
    test_aggregate_parser_propagates_error();
