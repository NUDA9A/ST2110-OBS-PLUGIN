...
AudioRtpPacketPolicy make_policy(AudioPcmWireFormat wire_format = AudioPcmWireFormat::L24, uint8_t payload_type = 96,
  ...
  auto policy = audio_rtp_packet_policy_from_rx_audio_config(make_level_a_rx_audio_config(payload_type, channel_count),
                                                             wire_format);
...
std::vector<uint8_t> make_rtp_packet(uint8_t payload_type, bool marker, uint16_t sequence_number, uint32_t timestamp,
                                     uint32_t ssrc, const std::vector<uint8_t> &payload) {
...
std::vector<uint8_t> make_rtp_packet_with_csrc_and_extension(uint8_t payload_type, bool marker,
                                                             uint16_t sequence_number, uint32_t timestamp,
                                                             uint32_t ssrc, uint32_t csrc,
  ...
  const std::size_t payload_offset = fixed_header_bytes + csrc_bytes + extension_header_bytes + extension_payload_bytes;
...
void tolerates_csrc_and_rtp_header_extension() {
  ...
  const auto packet =
      make_rtp_packet_with_csrc_and_extension(policy.payload_type, true, 100, 123456, 0x11111111, 0x22222222, payload);
...
void rejects_payload_size_mismatch() {
  ...
  const auto packet = make_rtp_packet(policy.payload_type, false, 1, 2, 3, payload);
