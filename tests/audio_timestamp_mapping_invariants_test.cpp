...
st2110::TimestampNs expected_ns_from_ticks(
    uint64_t ticks,
  ...
  const uint64_t remainder_ns =
      (remainder_ticks * kNsPerSecond) / rtp_clock_rate;

  return anchor_timestamp_ns + whole_ns + remainder_ns;
}

void expect_timestamp(
    const std::expected<st2110::TimestampNs, st2110::Error>& result,
    st2110::TimestampNs expected) {
...
void regular_48k_1ms_cadence_is_exact_and_monotonic() {
  ...
  for (uint32_t packet_index = 0; packet_index != 256; ++packet_index) {
    const uint32_t rtp_timestamp =
        static_cast<uint32_t>(90'000 + packet_index * 48u);
    const st2110::TimestampNs expected =
        anchor_ns + packet_index * kOneMillisecondNs;
...
void wraparound_preserves_monotonic_48k_cadence() {
  ...
  for (uint32_t packet_index = 0; packet_index != 16; ++packet_index) {
    const uint32_t rtp_timestamp =
        static_cast<uint32_t>(anchor_rtp + packet_index * 48u);
    const st2110::TimestampNs expected =
        packet_index * kOneMillisecondNs;
...
void long_running_stream_continues_beyond_one_32bit_rtp_epoch() {
  ...
  constexpr uint32_t second_rtp =
      static_cast<uint32_t>(large_forward_delta * 2ull);
  constexpr uint32_t third_rtp =
      static_cast<uint32_t>(large_forward_delta * 2ull + 96ull);
  ...
  const uint64_t third_accumulated_ticks =
      large_forward_delta * 2ull + 96ull;

  expect_timestamp(
      mapper.map(first_rtp),
      expected_ns_from_ticks(first_accumulated_ticks, kAudioClock48k));

  expect_timestamp(
      mapper.map(second_rtp),
      expected_ns_from_ticks(second_accumulated_ticks, kAudioClock48k));

  const auto third = mapper.map(third_rtp);
  expect_timestamp(
      third,
      expected_ns_from_ticks(third_accumulated_ticks, kAudioClock48k));
...
void playout_timing_preserves_media_cadence_with_constant_delay() {
  const st2110::AudioReceiverPlayoutTimingConfig cfg{
      .playout_delay_ns = 2'000'000ull
  };

  const auto first = st2110::audio_block_timing(
      100,
      10'000'000ull,
      cfg);
  ...
  const auto second = st2110::audio_block_timing(
      148,
      11'000'000ull,
      cfg);
  ...
  assert(second->media_timestamp_ns - first->media_timestamp_ns ==
         kOneMillisecondNs);
  assert(second->playout_timestamp_ns - first->playout_timestamp_ns ==
         kOneMillisecondNs);
}
}
