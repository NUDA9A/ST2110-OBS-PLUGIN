...
void regular_48k_1ms_cadence_is_exact_and_monotonic_in_configured_reference_mode() {
    const st2110::TimestampNs anchor_ns = 10'000'000'000ull;
    st2110::AudioRtpTimestampMapper mapper({
        .rtp_clock_rate = kAudioClock48k,
...
void regular_48k_1ms_cadence_is_exact_and_monotonic_from_first_observed_zero() {
    st2110::AudioRtpTimestampMapper mapper({
        .rtp_clock_rate = kAudioClock48k,
...
void wraparound_preserves_monotonic_48k_cadence_in_configured_reference_mode() {
    constexpr uint32_t anchor_rtp = 0xfffffff0u;

    st2110::AudioRtpTimestampMapper mapper({
        .rtp_clock_rate = kAudioClock48k,
        .initial_anchor_mode = st2110::RtpTimestampInitialAnchorMode::ConfiguredReference,
        .anchor_rtp_timestamp = anchor_rtp,
        .anchor_timestamp_ns = 0,
    });
...
void wraparound_preserves_monotonic_48k_cadence_from_first_observed_zero() {
    constexpr uint32_t first_observed_rtp = 0xfffffff0u;
    ...
    for (uint32_t packet_index = 0; packet_index != 16; ++packet_index) {
        const uint32_t rtp_timestamp = static_cast<uint32_t>(first_observed_rtp + packet_index * 48u);
        const st2110::TimestampNs expected = packet_index * kOneMillisecondNs;
...
void long_running_stream_continues_beyond_one_32bit_rtp_epoch_in_configured_reference_mode() {
    st2110::AudioRtpTimestampMapper mapper({
        .rtp_clock_rate = kAudioClock48k,
        .initial_anchor_mode = st2110::RtpTimestampInitialAnchorMode::ConfiguredReference,
        .anchor_rtp_timestamp = 0,
...
void long_running_stream_continues_beyond_one_32bit_rtp_epoch_from_first_observed_zero() {
    constexpr uint32_t first_observed_rtp = 123456u;
    ...
    expect_timestamp(mapper.map(first_observed_rtp), 0);

    constexpr uint64_t large_forward_delta = 0x7ffffff0ull;
    constexpr uint32_t second_rtp = static_cast<uint32_t>(first_observed_rtp + large_forward_delta);
    constexpr uint32_t third_rtp = static_cast<uint32_t>(first_observed_rtp + large_forward_delta * 2ull);
    constexpr uint32_t fourth_rtp = static_cast<uint32_t>(first_observed_rtp + large_forward_delta * 2ull + 96ull);

    const uint64_t second_accumulated_ticks = large_forward_delta;
    const uint64_t third_accumulated_ticks = large_forward_delta * 2ull;
    const uint64_t fourth_accumulated_ticks = large_forward_delta * 2ull + 96ull;

    expect_timestamp(mapper.map(second_rtp), expected_ns_from_ticks(second_accumulated_ticks, kAudioClock48k));
    expect_timestamp(mapper.map(third_rtp), expected_ns_from_ticks(third_accumulated_ticks, kAudioClock48k));

    const auto fourth = mapper.map(fourth_rtp);
    expect_timestamp(fourth, expected_ns_from_ticks(fourth_accumulated_ticks, kAudioClock48k));

    assert(*fourth > expected_ns_from_ticks(0xffffffffull, kAudioClock48k));
}
...
void reset_reanchors_mapping_and_discards_previous_cadence_state_in_configured_reference_mode() {
    st2110::AudioRtpTimestampMapper mapper({
        .rtp_clock_rate = kAudioClock48k,
        .initial_anchor_mode = st2110::RtpTimestampInitialAnchorMode::ConfiguredReference,
        .anchor_rtp_timestamp = 1'000,
    ...
    const auto reset_error = mapper.reset({
        .rtp_clock_rate = kAudioClock48k,
        .initial_anchor_mode = st2110::RtpTimestampInitialAnchorMode::ConfiguredReference,
        .anchor_rtp_timestamp = 50'000,
        .anchor_timestamp_ns = 5'000'000'000ull,
    });
...
void reset_reanchors_mapping_and_discards_previous_cadence_state_in_first_observed_zero_mode() {
    st2110::AudioRtpTimestampMapper mapper({
        .rtp_clock_rate = kAudioClock48k,
        .initial_anchor_mode = st2110::RtpTimestampInitialAnchorMode::FirstObservedBecomesLocalZero,
        .anchor_rtp_timestamp = 0,
    ...
    expect_timestamp(mapper.map(50'000), 0);
    expect_timestamp(mapper.map(50'048), kOneMillisecondNs);
}
...
int main() {
    regular_48k_1ms_cadence_is_exact_and_monotonic_in_configured_reference_mode();
    regular_48k_1ms_cadence_is_exact_and_monotonic_from_first_observed_zero();
    non_default_audio_clock_rate_uses_explicit_clock_rate();
    wraparound_preserves_monotonic_48k_cadence_in_configured_reference_mode();
    wraparound_preserves_monotonic_48k_cadence_from_first_observed_zero();
    long_running_stream_continues_beyond_one_32bit_rtp_epoch_in_configured_reference_mode();
    long_running_stream_continues_beyond_one_32bit_rtp_epoch_from_first_observed_zero();
    backward_or_ambiguous_delta_is_rejected_without_advancing_state();
    exact_half_range_delta_is_rejected_as_ambiguous();
    reset_reanchors_mapping_and_discards_previous_cadence_state_in_configured_reference_mode();
    reset_reanchors_mapping_and_discards_previous_cadence_state_in_first_observed_zero_mode();
