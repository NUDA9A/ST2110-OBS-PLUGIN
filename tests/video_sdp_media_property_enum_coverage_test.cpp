...
namespace {
    RawVideoSdpFmtpParameters make_base_raw_fmtp() {
        ...
        raw.width = 1920;
        raw.height = 1080;
        raw.exactframerate = RawVideoSdpExactFrameRate{
                .numerator = 25,
                .denominator = 1
        };
        raw.depth = 8;
    ...
    void assert_no_raw_token(const VideoSampling& sampling) {
        assert(!sampling.raw_token.has_value());
    }

    void assert_no_raw_token(const VideoColorimetry& colorimetry) {
        assert(!colorimetry.raw_token.has_value());
    }

    void assert_no_raw_token(const VideoTransferCharacteristicSystem& tcs) {
        assert(!tcs.raw_token.has_value());
    }
    ...
    void colorimetry_known_tokens_are_explicit_enums() {
        ...
        const Case cases[] = {
                {"BT601", VideoColorimetry::Known::Bt601},
                {"BT709", VideoColorimetry::Known::Bt709},
                {"BT2020", VideoColorimetry::Known::Bt2020},
                {"BT2100", VideoColorimetry::Known::Bt2100},
                {"ST2065-1", VideoColorimetry::Known::St2065_1},

                {"ST2065-3", VideoColorimetry::Known::St2065_3},
                {"UNSPECIFIED", VideoColorimetry::Known::Unspecified},
                {"XYZ", VideoColorimetry::Known::Xyz},
                {"ALPHA", VideoColorimetry::Known::Alpha},
        };
