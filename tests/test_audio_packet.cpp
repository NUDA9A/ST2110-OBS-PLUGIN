...
static_assert(std::is_same_v<
        decltype(st2110::AudioRtpPacketView{}.payload),
        st2110::ByteSpan>);

namespace {

    st2110::RxAudioConfig make_level_a_rx_config(
            uint16_t channels,
    ...
    st2110::RtpHeaderView make_rtp_header(
    ...
    void test_wire_format_sample_sizes_are_explicit() {
        auto l16 = st2110::audio_pcm_wire_sample_bytes(
                st2110::AudioPcmWireFormat::L16);
        assert(l16.has_value());
        assert(*l16 == 2);

        auto l24 = st2110::audio_pcm_wire_sample_bytes(
                st2110::AudioPcmWireFormat::L24);
        assert(l24.has_value());
        assert(*l24 == 3);

        auto invalid = st2110::audio_pcm_wire_sample_bytes(
                static_cast<st2110::AudioPcmWireFormat>(255));
    ...
    void test_policy_from_rx_audio_config_preserves_runtime_axes() {
        ...
        auto policy = st2110::audio_rtp_packet_policy_from_rx_audio_config(
                cfg,
                st2110::AudioPcmWireFormat::L24);
    ...
    void test_make_audio_packet_view_accepts_matching_payload() {
        ...
        auto packet = st2110::make_audio_rtp_packet_view(
                rtp,
                st2110::ByteSpan(payload.data(), payload.size()),
                *policy);
